...
export function AppRail({ current }: { current: AppId }) {
  ...
  return (
    <nav ref={ref} className={`rail${open ? ' is-open' : ''}`} aria-label="Switch app">
      <button
        type="button"
        ...
        onClick={() => setOpen((o) => !o)}
      >
        <span className="rail__brand">P8</span>
        <span className="rail__wordmark">Popul8</span>
      </button>

      <ul className="rail__apps">
        {APPS.map((a) => {
          ...
          return (
            <li key={a.id}>
              ...
              >
                <span className="rail__icon"><RailIcon id={a.id} /></span>
