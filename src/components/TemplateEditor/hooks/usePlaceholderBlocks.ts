...
export const buildPlaceholderBlocksFromSvg = (svg: SVGSVGElement) => {
  ...
  groupElements.forEach((group) => {
    ...
    const candidates: Element[] = [group, ...Array.from(group.querySelectorAll("*"))];
...
export const usePlaceholderBlocks = (content: string | null) =>
  useMemo(() => {
    ...
    if (!svg) return [];
    return buildPlaceholderBlocksFromSvg(svg as SVGSVGElement);
  }, [content]);
